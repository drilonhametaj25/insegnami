...
/**
 * Carica il materiale (scoped su classe+tenant) e verifica la titolarità:
 * admin ok; TEACHER solo se titolare della classe.
 */
async function loadForWrite(ctx: AuthContext, classId: string, materialId: string) {
  ...
  if (ctx.role === 'TEACHER') {
    ...
    if (!teacherId || material.lesson.class.teacherId !== teacherId) {
...
// GET /api/classes/[id]/materials/[materialId] — dettaglio; ?download=1 → file/URL
export async function GET(
  ...
  try {
    ...
    if (r) return r;
    console.error('Class material GET error:', error);
    return NextResponse.json({ error: 'Errore interno del server' }, { status: 500 });
  }
}

// PATCH /api/classes/[id]/materials/[materialId] — rename/descrizione
export async function PATCH(
  ...
  try {
    ...
    console.error('Class material PATCH error:', error);
    return NextResponse.json({ error: 'Errore interno del server' }, { status: 500 });
  }
}

// DELETE /api/classes/[id]/materials/[materialId]
export async function DELETE(
  ...
  try {
    ...
    const r = authError(error);
    if (r) return r;
    console.error('Class material DELETE error:', error);
    return NextResponse.json({ error: 'Errore interno del server' }, { status: 500 });
  }
}
